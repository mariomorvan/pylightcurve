...
def exoplanet_orbit(period, sma_over_rs, eccentricity, inclination, periastron, mid_time, time_array, ww=0):
    ...
    vv = 2 * torch.atan(np.sqrt((1 + eccentricity) / (1 - eccentricity)) * np.tan(u1 / 2))
    #
...
def transit_projected_distance(period, sma_over_rs, eccentricity, inclination, periastron, mid_time, time_array):
    ...
    return (torch.where(position_vector[0] < 0, - torch.ones_like(position_vector[0]), torch.ones_like(position_vector[0])) *
